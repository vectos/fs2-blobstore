...
case class Url(scheme: String, authority: Authority.Standard, path: Path.Plain) {
...
object Url {
  ...
  def standard(url: String): ValidatedNec[UrlParseError, Url] = parse(url)

  def standardF[F[_]: ApplicativeError[*[_], Throwable]](url: String): F[Url] = parseF(url)
        ...
        OptionT(matchRegex.leftWiden[UrlParseError])
          ...
          .map { pathPart =>
            if (!pathPart.startsWith("/"))
              Url("file", Authority.Standard.localhost, Path("/" + pathPart))
            else Url("file", Authority.Standard.localhost, Path(pathPart.stripPrefix("/")))
          }
          .toValidatedNec
