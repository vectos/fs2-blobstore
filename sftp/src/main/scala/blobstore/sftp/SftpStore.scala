...
class SftpStore[F[_]] private (
  ...
  def closeChannel(
    ch: ChannelSftp
  ): F[Unit] = semaphore match {
    case Some(s) => s.release.flatMap(_ => F.blocking(ch.disconnect()))
    case None    => ().pure
  }
  ...
  override def get[A](path: Path[A], chunkSize: Int): Stream[F, Byte] =
    Stream.resource(channelResource).flatMap { channel =>
      fs2.io.readInputStream(
        F.blocking(channel.get(path.show)),
        chunkSize = chunkSize,
        closeAfterUse = true
      )
    }
  ...
  override def remove[A](path: Path[A], recursive: Boolean = false): F[Unit] = {
    def recursiveRemove(path: Path[SftpFile]): F[Unit] = channelResource.use { channel =>
      val r =
        if (path.isDir) {
          list(path).evalMap(recursiveRemove) ++ Stream.eval(F.blocking(channel.rmdir(path.show)))
        } else Stream.eval(F.blocking(channel.rm(path.show)))
      r.compile.drain
    }
      ...
      _stat(path, channel).flatMap {
        case Some(p) =>
          if (recursive) recursiveRemove(p)
            ...
            if (p.isDir) F.blocking(channel.rmdir(p.show)) else F.blocking(channel.rm(p.show))
          }
        case None => ().pure[F]
      }
    }
  ...
  private def mkdirs[A](path: Path[A], channel: ChannelSftp): F[Unit] = {
    ...
    fs2.Stream.emits(path.segments.toList)
      ...
      .evalScan(root) { (acc, el) =>
        F.blocking(Try(channel.mkdir(acc.show))).as(acc / el)
      }
      .compile
      .drain
  }
  ...
  private def outputStreamResource[A](
    ...
    def put(channel: ChannelSftp): F[OutputStream] = {
      val newOrOverwrite =
        mkdirs(path, channel) >> F.blocking(channel.put(path.show, ChannelSftp.OVERWRITE))
      if (overwrite) {
        newOrOverwrite
      } else {
        F.blocking(channel.ls(path.show)).attempt.flatMap {
    ...
    def close(os: OutputStream): F[Unit] = F.blocking(os.close())
  ...
  def _stat[A](path: Path[A], channel: ChannelSftp): F[Option[Path[SftpFile]]] =
    F.blocking(channel.stat(path.show))
...
object SftpStore {
  ...
  /** Safely initialize SftpStore and disconnect ChannelSftp and Session upon finish.
    *
    * @param fSession F[Session] how to connect to SFTP server.
    */
  def apply[F[_]: Async](
    fSession: F[Session],
    maxChannels: Option[Long] = None,
    connectTimeout: Int = 10000
  )(implicit dispatcher: Dispatcher[F]): Resource[F, SftpStore[F]] =
