...
class SftpStore[F[_]] private (
  val authority: Authority.Standard,
  ...
  override def liftTo: Store[F, SftpFile] =
    liftTo((u: Url) => u.path.relative.valid)

  override def liftTo(g: Url => Validated[Throwable, Plain]): Store[F,SftpFile] =
    new Store.DelegatingStore[F, SftpFile](Right(this), g)
    ...
    if (maxChannels.exists(_ < 1)) {
      ...
      for {
        ...
        authority <- {
          ...
          Stream.eval(
            Authority.Standard.parse(session.getHost + port).leftMap(MultipleUrlValidationException.apply).liftTo[F]
          )
        }
