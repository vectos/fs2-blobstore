...
class S3Store[F[_]](
  ...
  def copy(src: Url, dst: Url, dstMeta: Option[S3MetaInfo]): F[Unit] = {
    val request = {
      val srcBucket = src.show
      val srcKey    = src.path.relative.show
      val dstBucket = dst.show
  ...
  override def stat(url: Url): Stream[F, Path[S3Blob]] =
    Stream.eval(liftJavaFuture(
      F.delay(s3.headObject(HeadObjectRequest.builder().bucket(url.show).key(url.path.relative.show).build()))
    ).map { resp =>
      Path.of(
        url.path.show,
        S3Blob(url.show, url.path.relative.show, new S3MetaInfo.HeadObjectResponseMetaInfo(resp).some)
      ).some
    }
      .recover {
        case _: NoSuchKeyException =>
          ...
          Path.of(url.path.show, S3Blob(url.show, url.path.relative.show, meta)).some
      }).unNone

  override def widen(implicit ME: MonadError[F, Throwable]): Store[F, S3Blob] =
    new DelegatingStore[F, S3Blob](Left(this))
}
